 ...
 *
function pcp_civicrm_navigationMenu(&$menu) {
  _pcp_civix_insert_navigation_menu($menu, NULL, array(
    'label' => ts('The Page', array('domain' => 'com.garethdylan.compucorp.contact.pcp')),
    'name' => 'the_page',
  ...
  _pcp_civix_navigationMenu($menu);
} // */
...
function pcp_civicrm_tabset($tabsetName, &$tabs, $context) {
  ...
  if ($tabsetName == 'civicrm/contact/view') {
	...
	while ($cpages->fetch()) $total++;
	
    $url = CRM_Utils_System::url( 'civicrm/contact/view/pcp',
                                  "reset=1&force=1&cid=$contactId" );
  	$tabs[] = array( 'id'    => 'contactPCPTab',
      'url'   => $url,
      'title' => 'Personal Campaign Pages <em>' . $total . '</em>',
      'weight' => 300,
    );
  }
}